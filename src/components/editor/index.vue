...
<script setup lang="ts" name="wngEditor">
  ...
  const state = reactive({
    editorConfig: {
      ...
      MENU_CONF: {
        uploadImage: {
          server: "api/fileUploadAndDownload/upload?noSave=1",
          timeout: 5 * 1000, // 5s,
          fieldName: "file",
          headers: {
            Accept: "text/x-json",
          ...
          customInsert(res, insertFn) {
            if (res.code === 0) {
              // The first parameter is the image URL, the second is the alt text
        ...
        uploadVideo: {
          ...
          allowedFileTypes: ["video/*"], // 自定义增加 http  header
          headers: {
            "x-token": Session.get("token") || "",
          },
